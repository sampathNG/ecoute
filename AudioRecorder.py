...
class DefaultSpeakerRecorder(BaseRecorder):
    def __init__(self):
        speaker_device_info = self.get_default_speaker()
        if not speaker_device_info:
            raise ValueError("[ERROR] No suitable output device found.")
        sample_rate = int(speaker_device_info['default_samplerate'])
        channels = speaker_device_info['max_input_channels']
        source = sr.Microphone(
            # speaker=True,
            device_index=speaker_device_info['index'],
    ...
    def get_default_speaker(self):
        """Uses sounddevice to fetch default speaker or loopback device."""
